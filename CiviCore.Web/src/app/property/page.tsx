            ...
            <main className="flex-grow pt-24 pb-section-gap max-w-container-max mx-auto w-full px-margin-mobile md:px-margin-desktop">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0">
                        {['all', 'available', 'rented', 'sold'].map(f => (
                            <button
                                key={f}
                                onClick={() => setFilter(f)}
                                className={`px-4 py-2 rounded-full font-label-md text-label-md transition-colors whitespace-nowrap ${filter === f
                    ...
                    <div className="relative w-full md:w-64">
                        <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" aria-hidden="true">search</span>
                        <input
                            type="text"
                ...
                {filteredProperties.length > 0 ? (
                            ...
                            return (
                                <div key={property.id} className="bg-surface dark:bg-primary-container rounded-2xl shadow-sm border border-border-subtle/50 dark:border-primary-container/50 overflow-hidden flex flex-col group">
                                    <div className="h-56 overflow-hidden relative">
                                        <img alt={property.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" src={url || 'https://lh3.googleusercontent.com/aida-public/AB6AXuBgQjp1zRS5bEW5kyJnCdZ1pKGLloPZ3aw2373P7YoJQxR38ckj8iywKwVpF_nfQx4Au2Pz06PyEa2J6icsa32JDPt89qDrrHUAgT8vrKg7v8uPHFMdQxiA_FQYzphaZlRonLb8CCp2GShtlfCPZgN3XvnCw3SgU_6a3cWY87CrCwnMHBFbgalIS-_U1l1WYLifoKpzrqiVFNudotHA7dWlTuGTlKnH8kl3CxjZk5nKDLy_ErWLfM8D79Ub5FFHIGLRaZddPTLri7c'} />
                                    ...
                                    <div className="p-6 flex flex-col flex-grow">
                                        ...
                                        <div className="grid grid-cols-3 gap-2 text-text-muted dark:text-on-primary/70 mb-6 border-t border-border-subtle dark:border-primary-container/50 pt-4">
                                            {property.bedrooms !== undefined && <div className="flex items-center gap-1"><span className="material-symbols-outlined text-sm" aria-hidden="true">bed</span> {property.bedrooms} Beds</div>}
                                            {property.bathrooms !== undefined && <div className="flex items-center gap-1"><span className="material-symbols-outlined text-sm" aria-hidden="true">shower</span> {property.bathrooms} Baths</div>}
                                            {property.landArea !== undefined && <div className="flex items-center gap-1"><span className="material-symbols-outlined text-sm" aria-hidden="true">square_foot</span> {property.landArea} m²</div>}
                                        </div>
                    ...
                    <div className="text-center py-16">
                        <span className="material-symbols-outlined text-6xl text-text-muted mb-4" aria-hidden="true">search_off</span>
