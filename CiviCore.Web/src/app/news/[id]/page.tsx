...
export default function NewsDetailPage() {
    ...
    useEffect(() => {
        ...
        fetch('/api/homepage/news')
            .then(res => res.json())
            .then(data => {
                const found = data.find((e: any) => e.id === id || e.url === `/news/${id}` || e.url === `/events/${id}` || e.url === id);
        ...
        return (
            <div className="bg-surface-container-lowest dark:bg-primary text-on-surface dark:text-on-primary font-body-md antialiased min-h-screen flex flex-col">
                ...
                <main className="flex-grow pt-32 text-center">
                    <h1 className="text-display-lg text-primary dark:text-primary-fixed-dim mb-4">Berita Tidak Ditemukan</h1>
                    <Link href="/news" className="text-[#b45309] hover:underline">Kembali ke Berita</Link>
                </main>
                    ...
                    <div className="bg-surface-container-lowest/90 dark:bg-primary-container/90 rounded-2xl shadow-xl shadow-primary-container/5 overflow-visible flex flex-col lg:flex-row border border-border-subtle dark:border-primary-container/50 backdrop-blur-xl">
                        ...
                        <div className="p-8 md:p-12 lg:w-2/3 flex flex-col justify-center">
                            <div className="flex items-center space-x-2 text-text-muted dark:text-on-primary/70 font-label-sm text-label-sm mb-8">
                                ...
                                <Link className="hover:text-primary dark:hover:text-primary-fixed-dim transition-colors" href="/news">Berita</Link>
                                <span className="material-symbols-outlined text-[14px]">chevron_right</span>
                                <span className="text-on-surface dark:text-on-primary truncate max-w-[200px] sm:max-w-xs">{newsItem.title}</span>
                            </div>
                        ...
                        {newsItem.action_type === 'youtube' && (
                            <div className="bg-surface-bright dark:bg-[#002117] p-8 md:p-12 lg:w-1/3 border-l border-border-subtle dark:border-primary-container flex flex-col justify-center relative overflow-hidden text-center">
                                ...
                                <p className="font-body-md text-body-md text-on-surface-variant dark:text-on-primary/70 mb-8 relative z-10">
                                    Saksikan siaran atau dokumentasi berita ini langsung di YouTube.
                                </p>
                                <a href={`https://www.youtube.com/watch?v=${newsItem.action_value}`} target="_blank" rel="noopener noreferrer" className="w-full bg-red-600 dark:bg-red-700 text-white hover:bg-red-700 dark:hover:bg-red-600 transition-colors py-3 px-6 rounded-lg font-label-md text-label-md shadow-md hover:shadow-lg flex items-center justify-center gap-2 relative z-10">
                                    Buka di YouTube <span className="material-symbols-outlined text-[18px]">open_in_new</span>
                                </a>
                            </div>
                        )}
                        {newsItem.action_type === 'link' && (
                            <div className="bg-surface-bright dark:bg-[#002117] p-8 md:p-12 lg:w-1/3 border-l border-border-subtle dark:border-primary-container flex flex-col justify-center relative overflow-hidden text-center">
                                ...
                                <p className="font-body-md text-body-md text-on-surface-variant dark:text-on-primary/70 mb-8">
                                    Informasi lebih lanjut atau pendaftaran untuk berita ini tersedia di situs eksternal.
                                </p>
                                <a href={newsItem.action_value} target="_blank" rel="noopener noreferrer" className="w-full bg-[#064e3b] dark:bg-primary-fixed-dim text-white dark:text-primary hover:bg-primary-container/90 dark:hover:bg-primary-fixed transition-colors py-3 px-6 rounded-lg font-label-md text-label-md shadow-md hover:shadow-lg flex items-center justify-center gap-2">
                                    Buka Tautan <span className="material-symbols-outlined text-[18px]">arrow_forward</span>
                                </a>
                            </div>
                        )}
                        {(!newsItem.action_type || newsItem.action_type === 'informational') && (
                            <div className="bg-surface-bright dark:bg-[#002117] p-8 md:p-12 lg:w-1/3 border-l border-border-subtle dark:border-primary-container flex flex-col justify-center items-center relative overflow-hidden text-center">
                                ...
                                <p className="font-body-md text-body-md text-on-surface-variant dark:text-on-primary/70 relative z-10">
                                    Berita ini bersifat informasional dan tidak memerlukan tindakan lebih lanjut.
                                </p>
                            </div>
                        )}
                    </div>
