...
import {
  ...
  TableWrapper, Th, FormInput, FormSelect } from
'../../admin/components/ui';
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <div className="relative">
        ...
        <div tabIndex={0} role="button" onClick={() => setOpen(true)} className={`block w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border ${error ? 'border-rose-500' : 'border-slate-200 dark:border-slate-700'} rounded-lg text-sm text-slate-900 dark:text-white cursor-pointer select-none flex justify-between items-center`} onKeyDown={(e) => {if (["Enter", " "].includes(e.key)) {e.preventDefault();e.currentTarget.click();}}}>
        ...
        <div className="absolute z-50 w-full mt-1 bg-white dark:bg-[#1B2236] border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl max-h-48 overflow-y-auto">
          ...
          filtered.map((o) =>
          <div key={o.value} tabIndex={0} role="button" onClick={() => {onChange(o.value);setOpen(false);setSearch('');}}
          className={`px-4 py-2.5 cursor-pointer text-sm border-b border-slate-100 dark:border-slate-700/50 last:border-0 transition-colors ${value === o.value ? 'bg-primary/10 text-primary font-bold' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`} onKeyDown={(e) => {if (["Enter", " "].includes(e.key)) {e.preventDefault();e.currentTarget.click();}}}>
                {o.label}
              </div>
          )}
          </div>
...
function UserModal({ open, onClose, onSaved, data, roles, householders }: {open: boolean;onClose: () => void;onSaved: () => void;data: User | null;roles: UserRole[];householders: any[];}) {
  ...
  useEffect(() => {
    if (data) setForm({ name: data.name || '', username: data.username || '', email: data.email || '', role_id: String(data.role_id) || '', password: '', password_confirmation: '', is_active: data.is_active !== false, HouseholderId: (data as any).householder_id || '' });else
    setForm({ name: '', username: '', email: '', role_id: '', password: '', password_confirmation: '', is_active: true, HouseholderId: '' });
  ...
  const handleSave = async () => {
    ...
    if (form.password && form.password !== form.password_confirmation) {
      errs.password_confirmation = t('users.error_password_mismatch', 'Passwords do not match.');
    }
    ...
    try {
      ...
      if (isEdit) await axios.put(`/api/users/${data!.id}`, payload);else
