...
export default function HomepageNewsForm() {
  ...
  const CATEGORY_OPTIONS = [
    ...
    const fetchNewsItem = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const r = await axios.get('/api/homepage/news');
            const newsItems = r.data || [];
            const ev = newsItems.find((e: any) => String(e.id) === String(id));
            if (ev) {
                setForm({
                    title: ev.title || '',
    ...
    const saveNewsItem = async () => {
        ...
        try {
            if (isEdit) {
                await axios.put(`/api/homepage/news/${id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
            } else {
                await axios.post('/api/homepage/news', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
            }
            navigate('/homepage/news');
        } catch (e: any) {
    ...
    if (loading) {
        return (
            <AdminLayout title={isEdit ? t('homepage.edit_news', 'Edit News') : t('homepage.add_news', 'Add News')}>
    ...
    return (
        <AdminLayout title={isEdit ? t('homepage.edit_news', 'Edit News') : t('homepage.add_news', 'Add News')} subtitle={t('homepage.manage_news_subtitle', 'Manage news details for the homepage')}>
            <div className="w-full lg:w-[80%] max-w-7xl mx-auto pb-12">
                <div className="flex items-center gap-4 mb-8">
                    <Link to="/homepage/news" className="text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer">
                        <span className="material-icons">arrow_back</span>
                    </Link>
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 dark:text-white leading-tight">{isEdit ? t('homepage.edit_news', 'Edit News') : t('homepage.add_new_news', 'Add New Article')}</h1>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <FormInput label={t('homepage.field_title', 'Title')} id="ev-title" value={form.title} onChange={e => {
                            ...
                            setForm(f => ({ ...f, title, url: (!f.url || f.url.startsWith('/news/') || f.url.startsWith('/events/')) ? slug : f.url }));
                        }} required error={errors.title} placeholder={t('homepage.placeholder_news_title', 'News title...')} />
                        <div>
                        ...
                        <FormInput label={t('homepage.field_location', 'Location')} id="ev-location" value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} placeholder={t('homepage.placeholder_location', 'e.g. Central Hall')} />
                        <FormSelect label={t('homepage.field_category', 'Category')} id="ev-cat" value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} options={CATEGORY_OPTIONS} placeholder={t('homepage.placeholder_none', 'None')} />
                        <FormSelect label={t('homepage.field_action_type', 'News Action / Content')} id="ev-action-type" value={form.action_type} onChange={e => setForm(f => ({ ...f, action_type: e.target.value, action_value: '' }))} options={[
                ...
                <div className="flex justify-end gap-3 mt-8">
                    ...
                    <button onClick={saveNewsItem} disabled={saving} className="px-6 py-2.5 rounded-xl bg-primary hover:opacity-90 text-white dark:text-surface text-sm font-bold shadow-lg shadow-primary/20 hover:scale-[1.02] hover:shadow-md transition-all duration-200 cursor-pointer disabled:opacity-60 disabled:hover:scale-100 disabled:cursor-not-allowed">
