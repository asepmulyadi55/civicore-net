...
export default function Reports() {
  ...
  const statCards = [
    { label: t('reports.stat_total_collected'), value: `Rp ${(stats?.total_collected || 0).toLocaleString('id-ID')}`, icon: 'payments', iconBg: 'bg-emerald-100 dark:bg-emerald-500/10', iconColor: 'text-emerald-500' },
    { label: t('reports.stat_total_pending'), value: `Rp ${(stats?.total_pending || 0).toLocaleString('id-ID')}`, icon: 'pending_actions', iconBg: 'bg-amber-100 dark:bg-amber-500/10', iconColor: 'text-amber-500' },
    { label: t('reports.stat_collection_rate'), value: `${stats?.collection_rate ?? 0}%`, icon: 'percent', iconBg: 'bg-primary/10', iconColor: 'text-primary' },
  ];
  ...
  return (
    <AdminLayout title={t('reports.title')}>
      ...
      <PageHeader title={t('reports.page_title')} subtitle={t('reports.page_subtitle', { year: filters.year })} />

      <FilterBar>
        <SearchInput value={filters.search} onChange={(v) => setFilter('search', v)} placeholder={t('reports.search_placeholder')} />
        <SelectFilter value={filters.year} onChange={(v) => setFilter('year', v)} options={years} placeholder={t('reports.all_years')} hidePlaceholderOption={true} />
        <SelectFilter value={filters.block_id} onChange={(v) => setFilter('block_id', v)} options={blockOptions} placeholder={t('reports.all_blocks')} />
        <button onClick={() => setFilters({ year: String(new Date().getFullYear()), block_id: '', search: '', page: 1 })}
          ...
          <span className="material-icons text-sm">close</span> {t('reports.btn_clear')}
        </button>
      </FilterBar>
        ...
        <TableWrapper>
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
              ...
              <Th>{t('reports.th_block_unit')}</Th>
              {MONTH_LABELS.map(m => <Th key={m} className="text-center !px-1.5">{t(`reports.months.${m}`)}</Th>)}
              <Th className="text-right">{t('reports.th_paid')}</Th>
              <Th className="text-right">{t('reports.th_pending')}</Th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {data.length === 0 ? (
              <tr><td colSpan={16}><EmptyState icon="summarize" title={t('reports.no_report_title')} subtitle={t('reports.no_report_subtitle')} /></td></tr>
            ) : data.map((r) => (
