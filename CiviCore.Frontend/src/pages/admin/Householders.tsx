...
import {
  ...
  SearchableSelect, CustomSelect } from
'../../admin/components/ui';
...
function HouseholderModal({ open, onClose, onSaved, data, blocks }) {
  ...
  useEffect(() => {
    if (data) setForm({ fullname: data.fullname || '', unitId: data.unitId || '', blockId: data.blockId || '', houseStatus: data.houseStatus ?? 0, isActive: data.isActive !== false, phone: data.phone || '', email: data.email || '' });else
    setForm({ fullname: '', unitId: '', blockId: '', houseStatus: 0, isActive: true, phone: '', email: '' });
    setErrors({});
  }, [data, open]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEdit) await axios.put(`/api/householders/${data.id}`, form);else
      await axios.post('/api/householders', form);
      onSaved();
      onClose();
    } catch (err: any) {
...
export default function Householders() {
  ...
  const doConfirm = async () => {
    ...
    try {
      if (confirm.type === 'delete') await axios.delete(`/api/householders/${confirm.item.id}`);else
      if (confirm.type === 'deactivate') await axios.patch(`/api/householders/${confirm.item.id}/deactivate`);else
      if (confirm.type === 'bulk') {await axios.delete('/api/householders/bulk', { data: { ids: selected } });setSelected([]);}
