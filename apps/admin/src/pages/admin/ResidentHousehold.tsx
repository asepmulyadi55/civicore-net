...
function ResidentModal({ open, onClose, onSaved, data }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) await axios.put(`/api/resident-portal/household/residents/${data.id}`, payload);else
      await axios.post('/api/resident-portal/household/residents', payload);
      onSaved();
      onClose();
    } catch (err: any) {
...
export default function ResidentHousehold() {
  ...
  return (
    <AdminLayout title={t('resident_household.title')} subtitle={t('resident_household.subtitle_manage', { block: data.blockName, unit: data.unitNumber })}>
      ...
      <div className="w-full lg:w-[80%] max-w-7xl mx-auto pb-12 mt-4">
